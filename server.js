...
app.get('/api/conversations', requireLogin, (req,res)=> {
  const box = ['inbox','archive','trash','all'].includes(req.query.box) ? req.query.box : 'inbox';
...
app.post('/api/conversations/:id/read', requireLogin, (req,res)=> {
  if (!convoById(req.params.id)) return res.status(404).json({ error:'not found' });
...
app.post('/api/conversations/:id/archive', requireLogin, (req,res)=> {
  if (!convoById(req.params.id)) return res.status(404).json({ error:'not found' });
  db.prepare("UPDATE conversations SET folder='archive', archived_at=?, updated_at=? WHERE id=?").run(nowIso(), nowIso(), req.params.id);
  emitConversation(req.params.id);
  res.json({ ok:true });
});
app.post('/api/conversations/:id/trash', requireLogin, (req,res)=> {
  if (!convoById(req.params.id)) return res.status(404).json({ error:'not found' });
  db.prepare("UPDATE conversations SET folder='trash', deleted_at=?, updated_at=? WHERE id=?").run(nowIso(), nowIso(), req.params.id);
  emitConversation(req.params.id);
  res.json({ ok:true });
});
app.post('/api/conversations/:id/restore', requireLogin, (req,res)=> {
  if (!convoById(req.params.id)) return res.status(404).json({ error:'not found' });
  db.prepare("UPDATE conversations SET folder='inbox', archived_at='', deleted_at='', updated_at=? WHERE id=?").run(nowIso(), req.params.id);
  emitConversation(req.params.id);
  res.json({ ok:true });
});
...
app.post('/api/widget/conversations/:id/messages', (req,res)=> {
  ...
  const info = db.prepare('INSERT INTO messages (conversation_id, sender_type, sender_name, sender_login, body, created_at) VALUES (?,?,?,?,?,?)').run(req.params.id, 'visitor', exists.visitor_name || '訪客', '', body, nowIso());
...
app.post('/api/conversations/:id/messages', requireLogin, (req,res)=> {
  ...
  const info = db.prepare('INSERT INTO messages (conversation_id, sender_type, sender_name, sender_login, body, created_at) VALUES (?,?,?,?,?,?)').run(req.params.id, 'agent', displayName, agent, body, nowIso());
  db.prepare('UPDATE conversations SET last_agent=?, last_agent_display=?, unread_count=0, updated_at=? WHERE id=?').run(agent, displayName, nowIso(), req.params.id);
  const msg = db.prepare('SELECT * FROM messages WHERE id=?').get(info.lastInsertRowid);
  io.to(req.params.id).emit('message', msg);
  emitConversation(req.params.id);
  res.json(msg);
